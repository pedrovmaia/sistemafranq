 ...
 * @license http://pro.workcontrol.com.br?p=3056 Bruno Henrique
 */
class Read {
    ...
    public function ExeRead($Tabela, $Termos = null, $ParseString = null) {
        ...
        $this->ExeReadInterno($Tabela);
        $sql = "";
        if ($this->Result) {
            if ($this->Result[0]["tabela_nivel"] == 1) {
                if (!empty($Termos)) {
                    $sql = "AND unidade_id = {$_SESSION["userSYSFranquia"]["unidade_padrao"]}";
                } else {
                    $sql = "WHERE unidade_id = {$_SESSION["userSYSFranquia"]["unidade_padrao"]}";
                }
            }
        }
        ...
        $this->Select = "SELECT * FROM {$Tabela} {$Termos} {$sql}";
        $this->Execute();
    }

    public function ExeReadInterno($Tabela) {
        $this->Select = "SELECT * FROM sys_tabelas WHERE tabela_nome = '{$Tabela}'";
        $this->Connect();
        try {
            $this->Read->execute();
