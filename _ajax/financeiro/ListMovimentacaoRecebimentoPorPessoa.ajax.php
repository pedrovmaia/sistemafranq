    ...
    case 'list':
        ...
        $Read->FullRead("SELECT movimentacao.movimentacao_id, movimentacao.movimentacao_descricao,
                 ...
                 LEFT OUTER JOIN sys_pessoas pessoa ON pessoa_id = movimentacao.movimentacao_pessoa_id
                  WHERE movimentacao.movimentacao_tipo_id = 2 AND pessoa.pessoa_id = :id AND movimentacao.unidade_id = :unidade", "id={$Id}&unidade={$_SESSION['userSYSFranquia']['unidade_padrao']}");
        if($Read->getResult()){
            ...
            foreach ($Read->getResult() as $Result){
                ...
                array_push($arr, array(
                        ...
                        "acoes" => "<span class='td-actions'>" . ($_SESSION['permissao']['ler'] == 1 ? "<a rel='tooltip' href='painel.php?exe=contasreceber/ver_movimentacao_recebimento&id={$Result["movimentacao_id"]}' title='Ver' class='btn btn-primary btn-link mr-1'><i class='material-icons'>visibility</i></a>" : '') . "</span>"
                    )
                );
            }
            $manage['rows'] = $arr;
