...
switch ($action):
    ...
    case 'PessoasEditar':
        ...
        elseif ($PostData['catalogo_endereco'] != true):
            ...
            $Read->FullRead("SELECT pessoa_nome, pessoa_id FROM sys_pessoas WHERE pessoa_cpf = :cpf AND pessoa_id != :id", "cpf={$PostData['pessoa_cpf']}&id={$Id}");
            if($Read->getResult()) {
                ...
                for($i = 0; $i <= $quantidade_telefone; $i++){
                    if(isset($PostData['tipo_telefone_' . $i])){
                        if (isset($PostData['telid_' . $i])){
                            ...
                            $jSON['redirect'] = "painel.php?exe=crm/cadastro_pessoas&id=" . $Id;
                        } else {
                            $ArrTelefones[] = Array(
                                'pessoa_id' => $Id,
    ...
    case 'PessoasAdd':
        ...
        elseif ($PostData['catalogo_endereco'] != true):
            ...
            $Read->FullRead("SELECT pessoa_nome, pessoa_id FROM sys_pessoas WHERE pessoa_cpf = :cpf", "cpf={$PostData['pessoa_cpf']}");
            if($Read->getResult()) {
                $jSON['error'] = 'CPF informado já foi cadastrado!';
                echo json_encode($jSON);
                die;
            ...
            $jSON['success'] = 'Seu registro foi salvo com sucesso!';
            $jSON['redirect'] = "painel.php?exe=crm/filtro_pessoas";
        endif;
