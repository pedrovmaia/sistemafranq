...
switch ($action):
    ...
    case 'list':
        ...
        $Read->FullRead("SELECT projeto.projeto_id,
                ...
                LEFT OUTER JOIN sys_estagio_produto produto ON produto.estagio_produto_id = projeto.projeto_produto_id 
                LEFT OUTER JOIN sys_tipo_projeto tipo_projeto ON tipo_projeto.tipo_projeto_id = projeto.projeto_tipo_id WHERE projeto.unidade_id = :unidade AND projeto.projeto_data_inicio > NOW() GROUP BY projeto.projeto_id", "unidade={$_SESSION['userSYSFranquia']['unidade_padrao']}");
        if($Read->getResult()){
            ...
            foreach ($Read->getResult() as $Result){
                ...
                array_push($arr, array(
                        ...
                        "curso" => $Result['estagio_produto_nome'],
                        ...
                        "acoes" => "<span class='td-actions'>" . ($_SESSION['permissao']['ler'] == 1 ? "<a rel='tooltip' href='painel.php?exe=escola/turma/ver_turma&id={$Result["projeto_id"]}' title='Ver' class='btn btn-primary btn-link mr-1'><i class='material-icons'>visibility</i></a>" : '') . " " . ($_SESSION['permissao']['alterar'] == 1 ? "<a rel='tooltip' href='painel.php?exe=escola/turma/cadastro_turma&id={$Result["projeto_id"]}' title='Editar' class='btn btn-primary btn-link mr-1'><i class='material-icons'>mode_edit</i></a>" : '') . "</span>"
                    )
                );
            }
            $manage['rows'] = $arr;
