...
switch ($action):
    ...
    case 'ColaboradorEditar':
        ...
        elseif (empty($PostData['pessoa_cargo_id'])):
            $jSON['error'] = 'Informe um cargo!';
        elseif (empty($PostData['pessoa_acesso_id'])):
            $jSON['error'] = 'Informe um nível acesso!';
        else:

            $Id = $PostData['pessoa_id'];
            unset($PostData['pessoa_id']);

            $Read->FullRead("SELECT pessoa_nome, pessoa_id FROM sys_pessoas WHERE pessoa_cpf = :cpf AND pessoa_id != :id AND pessoa_tipo_id = 6", "cpf={$PostData['pessoa_cpf']}&id={$Id}");
            if($Read->getResult()) {
            ...
            $Read->FullRead("SELECT pessoa_nome, pessoa_id FROM sys_pessoas WHERE pessoa_email = :cpf AND pessoa_id != :id", "cpf={$PostData['pessoa_email']}&id={$Id}");
            if($Read->getResult()) {
            ...
            $UpdateData['pessoa_cargo_id'] = $PostData['pessoa_cargo_id'];
            ...
            if($PostData['acesso_sistema']){
                if (!empty($PostData['acesso_senha'])){

                    $CreateDataAcesso['acesso_senha'] = hash('sha512', $PostData['acesso_senha']);
                    $Update->ExeUpdate("sys_acesso_pessoas", $CreateDataAcesso, "WHERE acesso_pessoa_id = :id", "id={$Id}");
            ...
            if($PostData['acesso_portal']){
                if (!empty($PostData['acesso_senha'])){
                    ...
                    $CreateDataAcessoPortal['escola_acesso_senha'] = hash('sha512', $PostData['acesso_senha']);
                    $Update->ExeUpdate("escola_acesso", $CreateDataAcessoPortal, "WHERE acesso_pessoa_id = :id", "id={$Id}");
                }
            }
    ...
    case 'ColaboradorAdd':
        ...
        elseif (empty($PostData['pessoa_cargo_id'])):
            $jSON['error'] = 'Informe um cargo!';
        elseif (empty($PostData['pessoa_acesso_id'])):
            $jSON['error'] = 'Informe um nível de acesso!';
        else:

            $Read->FullRead("SELECT pessoa_nome, pessoa_id FROM sys_pessoas WHERE pessoa_cpf = :cpf AND pessoa_tipo_id = 6", "cpf={$PostData['pessoa_cpf']}");
            if($Read->getResult()) {
                $jSON['error'] = 'CPF informado já foi cadastrado!';
                echo json_encode($jSON);
                die;
            ...
            $Read->FullRead("SELECT pessoa_nome, pessoa_id FROM sys_pessoas WHERE pessoa_email = :cpf", "cpf={$PostData['pessoa_email']}");
            if($Read->getResult()) {
                $jSON['error'] = 'Email informado já foi cadastrado!';
                echo json_encode($jSON);
                die;
            }
            ...
            if($PostData['acesso_sistema'] || $PostData['acesso_portal']){
                if (empty($PostData['acesso_senha'])){
                    $jSON['error'] = 'Informe um nível de acesso!';
                    echo json_encode($jSON);
                    die;
            ...
            $CreateData['pessoa_cargo_id'] = $PostData['pessoa_cargo_id'];
                ...
                for($i = 0; $i <= $quantidade_telefone; $i++){
                    if(isset($PostData['tipo_telefone_' . $i])){
                        ...
                        $ArrTelefones[] = Array(
                            ...
                            'telefone' => $PostData['telefone_' . $i],
                            'ramal' => $PostData['ramal_' . $i],
                            'operadora' => (isset($PostData['operadora_' . $i]) ? $PostData['operadora_' . $i] : null ),
                            'observacao' => $PostData['observacao_' . $i],
                            'unidade_id' => $_SESSION["userSYSFranquia"]["unidade_padrao"],
                            'data_criacao' => date('Y-m-d H:i:s'),
                            'usuario_criacao' => $_SESSION["userSYSFranquia"]["pessoa_id"]
    ...
    case 'infos_tel':
        ...
        $clone .= "<select required class='form-control' name='tipo_telefone_".$PostData['numero']."'><option value=''>Selecione um Tipo</option>".$TipoOption."</select>";
        $clone .= "</td><td class='pt-3-half'><input required type='tel' name='telefone_".$PostData['numero']."' placeholder='(00) 0000-0000' class='form-control formPhone'></td>";
